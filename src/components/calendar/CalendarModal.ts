...
export const CalendarModal = (halls: Hall[]) => {
  ...
  const updateMonthYear = () => {
    monthYearDisplay.textContent = `${monthNames[activeMonth]} ${year}`;
  };
  ...
  const generateDays = (monthIndex: number) => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      const actualDay = document.querySelector(".calendar-day.active");
  ...
  //         dayCell.classList.add("active");
  //         dayCell.addEventListener("click", () => openModal(hall));
  //       }
